...
data:
  ...
  bundledCode: "#line 1 \"test/library_checker/graph/frequency_table_of_tree_distance.test.cpp\"\
    ...
    \ UNIQUE(x) sort(all(x)), x.erase(unique(all(x)), x.end())\n#line 3 \"test/library_checker/graph/frequency_table_of_tree_distance.test.cpp\"\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/frequency_table_of_tree_distance\"\
    ...
    \ len(data)) {\r\n      int n = len(data[i]) + 1;\r\n      vi A(n);\r\n      FOR(j,\
    \ len(data[i])) A[data[i][j].se]++;\r\n      while (A.back() == 0) A.pop_back();\r\
    \n      auto B = conv_square_ll(A);\r\n      FOR(j, len(B)) ANS[j] += (i == 0\
